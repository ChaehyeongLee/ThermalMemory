...
def model_arctan(x, coeff): 
    """
    Arctangent model for autocorrelation function fitting.
    
    Args:
        x: lag time array
        coeff: arctangent coefficient to be fitted
    
    Returns:
        Modeled autocorrelation values
    """
    return 1-0.5*np.arctan(coeff*x)

def arctan_tau(year_idx):
    """
    Calculate thermal memory using arctangent model for a specific year.
    
    Args:
        year_idx: year to process (e.g., 1982, 1983, ...)
    
    This function:
    1. Extracts annual SST anomaly data
    2. Calculates autocorrelation functions for different lag lengths
    3. Fits arctangent model to each autocorrelation function
    4. Converts fitted parameters to e-folding timescales
    5. Saves results to NetCDF file
    """
    global ssta, f_dir

    # Set up date range for the target year
    ...
    # Initialize autocorrelation arrays for different lag lengths
    ...
    tau = np.zeros([5,180,360])*np.NaN  # Thermal memory timescales

    lag_data = np.array([2,5,10,20,30])  # Different lag lengths to analyze
    fit_lags = np.arange(30)  # Lag array for fitting
    ...
    for lat in range(180):
        for lon in range(360):
            ...
            if np.isnan(np.sum(ssta_target[:,lat,lon])) == False:
                # Calculate autocorrelation functions for different lag lengths
                ...
                # Fit arctangent model to each autocorrelation function
                ...
                # Convert fitted coefficients to e-folding timescales
    ...
    tau_xr = xr.Dataset(data_vars=dict(
        ...
        coords={'lag':lag_data,'time':time_data,'lat':lat_data,'lon':lon_data},
